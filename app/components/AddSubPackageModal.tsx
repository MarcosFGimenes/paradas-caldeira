...
import { SubPackageService } from "@/app/lib/firestore";

interface AddSubPackageModalProps {
  packageId: string;
  onClose: () => void;
  onCreated?: (id: string) => void;
}

const AddSubPackageModal: React.FC<AddSubPackageModalProps> = ({ packageId, onClose, onCreated }) => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const id = await SubPackageService.create({ name, description, packageId });
      onCreated?.(id);
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Erro ao criar subpacote.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-white/10 bg-slate-900/90 p-6 shadow-2xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-sm uppercase tracking-wide text-slate-400">Novo subpacote</p>
          <h2 className="text-xl font-semibold text-white">Adicionar subpacote</h2>
          <p className="text-sm text-slate-400">Separe serviços por empresa, etapa ou lote.</p>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            className="w-full rounded-lg border border-white/10 bg-slate-800 px-3 py-2 text-sm text-white focus:border-emerald-400/60 focus:outline-none"
            placeholder="Detalhe os serviços ou a empresa responsável"
            rows={3}
          />
        </div>
...
export default AddSubPackageModal;
