...
export default function PackagePage() {
  ...
  const handleSubPackageRemoved = async (sub: SubPackage) => {
    ...
    try {
      await SubPackageService.removeWithChildren(sub.id);
      setSubpackages((prev) => {
        const nextList = prev.filter((item) => item.id !== sub.id);
        setSelectedSubId((prevSelected) => (prevSelected === sub.id ? null : prevSelected));
          ...
          <section>
            {selectedSubPackage ? (
              <SubPackageView
                ...
                loading={selectedSubPackage.id ? loadingWorkOrders[selectedSubPackage.id] : false}
                onLoadRequest={
                  selectedSubPackage.id
                    ? () =>
                        !loadingWorkOrders[selectedSubPackage.id] &&
                        !workOrdersBySub[selectedSubPackage.id] &&
                        loadWorkOrders(selectedSubPackage.id)
                    : undefined
                }
